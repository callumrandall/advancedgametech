...
bool CollisionDetection::SphereIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
	...
	/*if (collisionInfo.a->GetName() == "Pickup" && collisionInfo.b->GetName() == "Enemy") {
		return false;
	}

	if (collisionInfo.a->GetName() == "Enemy" && collisionInfo.b->GetName() == "Pickup") {
		return false;
	}*/
	...
	if (deltaLength < radii) {
		...
		if (collisionInfo.a->GetName() == "Player" && collisionInfo.b->GetName() == "Enemy") {
			//std::cout << "Death" << std::endl;
			collisionInfo.a->dead = true;
		}

		if (collisionInfo.a->GetName() == "Enemy" && collisionInfo.b->GetName() == "Player") {
			//std::cout << "Death2" << std::endl;
			collisionInfo.b->dead = true;
		}

		if (collisionInfo.a->GetName() == "Player" && collisionInfo.b->GetName() == "Finish") {
			collisionInfo.a->game1fin = true;
		}

		if (collisionInfo.a->GetName() == "Finish" && collisionInfo.b->GetName() == "Player") {
			collisionInfo.b->game1fin = true;
		}
...
bool CollisionDetection::SphereCapsuleIntersection(
	...
	Vector3 topPos = tempPos + capsulePos; //top sphere pos
	Vector3 bottomPos = -tempPos + capsulePos; //bottom sphere pos

	Vector3 direction = (bottomPos - topPos).Normalised(); //direction of capsule
	Vector3 delta = (spherePos - topPos);
	float sphereProj = Vector3::Dot(delta, direction); //projection from sphere to capsule

	
	Vector3 closestP = topPos + (direction * sphereProj); //closest point on capsule to sphere
	float distance = (closestP - topPos).Length();

	Vector3 localP;
	if ((capsuleSize * 2) <= distance) { // local point is the bottom sphere
		localP = bottomPos; 
	}
	else if (sphereProj < 0) { // local point is top sphere
		localP = topPos;
	}
	else {
		localP = closestP; // else its the closest point on capsule
	}

	float distanceToLocalP = (localP - spherePos).Length();
	float radii = capsuleR + sphereR;

	// basically sphere sphere intersection
	// if objects are further apart than their radii combined
	if (distanceToLocalP > radii)
		return false; // too far apart to collide
	...
	const AABBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	if (distance < capsuleR) {//yes , we’re colliding!
	...
	const OBBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	// Get top and bottom sphere positions on capsule
	Vector3 tempPos = Vector3(sin(capsuleNormal.x) * capsuleSize, cos(capsuleNormal.y) * capsuleSize, sin(capsuleNormal.z) * capsuleSize);
	...
	if ((capsuleSize * 2) <= distance) {
		localP = bottomPos; //bottom sphere
	}
	else if (proj < 0) {
		localP = topPos; //top sphere
	}
	else {
		localP = closestPoint; //else its the closest point
	}


	// similar to obb sphere from here
