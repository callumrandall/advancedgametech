...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -75.0f, 0.0f));
}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float jFriction = (-(mu * (Vector3::Dot(contactVelocity, tNormal)))) / (totalMass + Vector3::Dot(frictionA + frictionB, tNormal));
	float j = (-(1.0f + cRestitution) * impulseForce) / (totalMass + angularEffect);

	Vector3 fullImpulse = p.normal * j;
	Vector3 frictionImpulse = tNormal * jFriction;
...
void PhysicsSystem::BroadPhase() {
	...
	QuadTree < GameObject* > tree(Vector2(1024, 1024), 25, 25);
